...
export default {
    ...
    run: async function (socket: any, data: any) {
        try {
            ...
            if(userDocument.friends_requests_sent.includes(friend.user_id)) return socket.emit("user.friend.add", { error: "You already sent a friend request to " + friend.username });
            ...
            if(userDocument.friends_requests_received.includes(friend.user_id)) { // if the user has a friend request from the friend
                ...
                return socket.to(friend.user_id).emit("user.friend.add", { success: "You are now friends with " + user.username }); // send a success message to the friend
                ...
                await userDocument.save(); // save the user document
                await friend.save(); // save the friend document
                socket.emit("user.friend.add", { success: "You sent a friend request to " + friend.username }); // send a success message to the user
                socket.to(friend.user_id).emit("user.friend.add", { success: user.username + " sent you a friend request" }); // send a success message to the friend
            }
